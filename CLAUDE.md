...
**Overriding rule (2026-07-28, Jay): body text must run continuously, with no dead gaps.** `ke-clear` forcing a heading/paragraph to jump past a still-tall float creates exactly the kind of visible blank void this rule forbids — prefer sizing the float itself (anywhere from ~1/4 to ~3/4 article width, whatever the accompanying text needs) so text keeps flowing immediately, over inserting a clear that skips ahead. Reserve `ke-clear` for places with no visible gap it would create (e.g. a genuinely empty tail after the last float on a page). This rule takes precedence over the sizing guidance in rule 3 below when the two conflict — narrow-line and continuity concerns should both be solved by adjusting float width/placement, not by clearing.

1. Never split a heading from its own body text — a `.ke-sidebar`/`.ke-figure` can't be the first thing after a heading.
2. Avoid side-by-side images unless they're a confirmed obviously-paired photo set (e.g. front/back) — use `.ke-figure-row` for those; otherwise force stacking with same-side floats (not `ke-clear` — see the overriding rule above).
3. No wrapped line of body text narrower than ~3 words (~200px floor). Float smaller companion images too rather than leaving them to defer past a whole float; default to `.ke-figure-right/-left` (46%), drop to `-third`/`-quarter`/`-column` when a sidebar is actively competing — `.ke-figure-column` (fills the exact leftover width beside a `data-width="wide"` sidebar, ~42%) avoids the narrow-line trap that a same-side `-quarter` figure can fall into when it turns out to still fit alongside the sidebar instead of deferring below it.
...
13. **Wide data tables are handled automatically — don't hand-build one.** Any `article` table with 4+ columns is transformed at runtime by `BaseLayout.astro`: below 700px each row becomes a card (row label as heading, header cells supplying each value's label); above it, a trailing prose column averaging 60+ characters lifts out into a full-width note row under its category and numeric cells get tinted by value. Short-value tables are left alone by that guard. Write a plain markdown pipe table and let it apply.
14. When adding a float/width variant, add it to the `max-width: 700px` block *by name*. Attribute and compound selectors (`.ke-sidebar[data-width='wide']`, `.ke-figure-third.ke-figure-left`) out-specify a bare class, so a mobile override that doesn't name them is silently ignored — this shipped broken for months.
