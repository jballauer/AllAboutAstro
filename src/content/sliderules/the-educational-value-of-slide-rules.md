...
And therein lies the polarizing nature of math.   How many students give up before the reward?  
"It's the abstractions that make math challenging, but it's also that aspect which makes it rewarding.  And therein lies the polarizing nature of math." 
...
Not only has this calculator's technology been usurped by phone apps and computers, but it is also the least convincing visual or pedagogical way to understand math.   

Students could now be using, for free, Desmos or Wolfram Alpha on the smartphone they already own.  Such "apps" cannot only give you solutions like the calculator, but can show you step-by-step how to do it, as well as showing you extra information and extensions to the learning.  Moreover, I can use Desmos to quickly and easily set-up a slider on a value to show visually its effect on a function.   They, and more powerful PC solutions like a programming language (e.g. Python or MatLab) and even an Excel Spreadsheet, cannot only solve problems, but they also let students create their own problems to solve.  

The idea for this is simple - if you give productive people a new tool, then they will SEEK and CREATE opportunities to use the tool. 

The only time I ever saw students use our calculators in this way was before smartphones when a few enterprising students would dive into the calculator's programmability, but anybody who has done that on a TI calculator can tell you how unintuitive and complicated that is.   Simply put, the calculators in my classroom are not a good tool by today's standards. 

So why use them?  In Texas, our standards require us to use a "graphing calculator" at the high school level.   As such, Texas students will not be reaching for a "Casio" when your state's name is on education's most popular calculator.  No matter that it was made obsolete by the first iPhone 15 years ago.  Not to bury the lead, but we'll refrain from asking "why" here.   Let us just say that a lot of money changes hands in education, with teachers not seeing much of it. 

Note: While writing this, my district has supplied me with new TI-84 Plus CE calculators, with color LCD, self-charging, and even with Python programming functionality.   It's much better than what I had, and they are appreciated, but they are still not up to the capabilities of a good, free phone app!
As a Manipulative
...
Students today feel that if they had a calculator, then everything would be miraculously better.   
...
On a slide rule, the C and D scales (labelled on the left of the scales), provide students the ability to do division. In this case, with the 8 on the D scale aligned with the 7 on the C scale, the result of 8 
÷7
÷
7
 can be read at the D scale beneath the "1" or "left index" of the C scale, as shown here on the hairline of the cursor. This reading of 114 (and a little more) are the significant figures for any of the computations at the left. It becomes YOUR job to supply the necessary decimal.
...
It was quite commonplace for school children to estimate a problem such as "80 divided by 7"  as being "a little more" than 10, simply because if 
70÷7=10
70
÷
7
...
10
1
.   As such, they would have known, when multiplying those numbers, that the product of same-base exponentials adds the powers:   
10
2
×
10
...
10
3
 would have told the student to drag the decimal to the right three places, yielding approximately 4400 as a result (we must apply the last zero). 
...
Here on this K&E Deci-Lon 10 slide rule, I've circled 3 on the D scale and have aligned it with the "R" gauge mark on the C scale. This gauge mark represents 57.2, which here is being divided into the 3. Note under the hairline on the cursor, 524 can be read. These significant figures require the user to know that .0524 would be the proper reading for computing the sine or tangent value of 3 degrees. Students of the slide rule era would have known that sine and tangent at these angle sizes would require this decimal placement...and they would know that at such small angle sizes, the sine and tangent functions are mostly equivalent.
...
​
​Note:  For those needing a quick reminder, the log of a number is the value required to raise a base, in this case base 10, to yield the argument (the number in parentheses).  Said as an exponential, 
10
.477
=3
10
...
log(30)=log(3×10)=log3+log10=0.477(mantissa)+1(characteristic)=1.477
𝑙
𝑜
𝑔
(
...
log(3000)=log(3×1000)=log3+log1000=0.477(mantissa)+3(characteristic)=3.477
𝑙
𝑜
𝑔
(
...
Figure 4 - How about division? It's essentially the inverse of multiplication, meaning that here you align the numbers to be divided and read the answer off of the index. The scales are set to show 9 / 4 = 2.25. Here I have divided up the logs between 2 and 3 to provide more resolution for the decimal quotient.
...
Do the same with the remainder of the table, yielding a number line from 1 to 10.  Get as close as possible with your percentage approximations for more accuracy.  For example, if 
log(3)=.477
𝑙
𝑜
𝑔
...
=
.477
, then 47.7% would be a skosh shy of 4 3/4" on the ruler.   Note that the gap between 0 and 1 is far greater than the gap between 9 and 10.  This is as you should expect with a logarithmic scale.   Use the first scale to create another similar to it in Figure 1. 
...
Put the left index of one of the scales onto the 2 of the other scale, as in Figure 2. Then, measure 3 on the top rule and look at where that value falls on the bottom rule.  Observe that the result is 6.   Congratulations, you just performed the computation 
2×3=6
2
×
3
=
6
.  In addition, observe that you just performed the following computations as well:
...
Students seldom have that opportunity, and when it does happen, it's often too late in the average student's journey.   Once they've learned how about numbers, we put a calculator in their hand and the investigation about those numbers usually ends.   And because students no longer need to use long division or multiplication on paper, they no longer remember those processes once they are needed later to factor via polynomial division, find slant asymptotes, multiply binomials, or to take the occasional integral in calculus. 
...
≈
2.72
?   These can also be represented to students as a good visual of those values relative to others.  Similarly, a teacher could have students put their own custom values on the scale (often called "gauges").  For example, some square roots like 
2
–
√
,
...
√
10
 routinely come up in class whereas a decimal equivalence can be useful when memorized.   Even with a paper copy of the above scale, having students point out a variety of decimals to the hundredth's place will pay dividends.  Then having them find 1.41, 1.73, and 3.16 and labeling them with the above square roots respectively, not only will there be a better sense of the placement of these values compared to their integer arguments, but they can discover first hand how closely approximate the square root of 10 actually is to 
π
𝜋
!*
...
Thus, with one setting of the slide, you have computations for 
8÷5=1.6
8
÷
5
...
=
.625
, which are also the decimal reciprocals of those values.  Important to note that those same values are the quotient for ANY two numbers opposing each other on the C and D scales.  So, 2 on the C and 3.2 on the D can be related as 
3.2÷2=1.6
3.2
÷
2
...
=
.625
.  Thus, we see a setting of the rule for ALL ratios that equate to either 1.6 or 0.625.    Again, this is for any aligned pairing of numbers, such as 2.5 and 4; 3 and 4.8; and even 40 and 64, considering that a number like "4" on a scale also represents 40 or 400 or 0.400, if you are so inclined. 
...
...whereas under the cursor the "1" or "left-index" of the C scale is set over the 1.9 mark on the D scale.   From there, all aligned numbers would represent fractions equivalent to 1.9 when reading the D scale over the C scale.   Quickly here, 
2.85
1.5
2.85
1.5
...
22
7
 as shown under the cursor line.  
Note that  
22
7
22
...
710
226
 approximates pi correctly up to 3.141592.  That is 8-significant figures if you aren't counting.  Give that one a try on your slide rule! 

This gives you an idea of the usefulness of the "folding scales" like CF and DF.   Having the index in the center of the rule, it assure that there are no "off-the-rule" computations when using the wrong index on the normal scales.   Similarly, because the scale is folded at pi, then you always have the product of 
n×π
𝑛
×
𝜋
...
n
𝑛
 is on the D scale and the product is read off the DF scale (no movement of the slide is necessary).  As such, this makes quick work of computations involving the circumference and area of circles.

*Historical Note: Because 
10
−
−
√
...
π
𝜋
, many slide rules from other makers began their folded scales at the square root of 10 instead of pi. 
...
×
×
 ____ = 16, with the understanding that the numbers in the blanks had to be the same number (thinking of the length and width required to make a "square").  And after recalling their 3rd grade multiplication tables, would have realized that 
4×4
4
×
4
 was the multiplication that yielded a 16.  Now, note the image below: ​
...
While this activity works to introduce kids to functional relationships of squares and square roots, it's important to see that algebraic associations are built as well.  Perhaps you noticed that this slide rule is also self-documented on the right side, marked by 
x
𝑥
 on the D scale and 
x
...
𝑥
2
 on the A scale?  Just as the "blank" can change in the word statements, students could see that the 
x
𝑥
 varies as well, and could be assigned to whatever made the word statement true.   A true transition from the "holder of a value" to " 
x
𝑥
 ​ as a variable" is being made here. I would challenge a teacher to find a more intuitive and concrete way to do that for your 6th grade students!  

The "Darmstadt" arrangement of slide rule scales, which became standardized on a variety of mostly European slide rules - this Aristo rule is German -  also allows students to build intuition about other functions as well.   In the above picture, note that the K scale is marked clearly for cubes (or cubed roots).  So, 
4
3
=64
4
...
=
64
, completing a word sentence of ____ 
×
×
 ____ 
×
...
x
𝑥
" becomes the blank.   Note as well on the slide that reciprocals 
(1/x)
(
1
/
𝑥
 ...
 can be discovered off of the "C-inverted" (or CI scale).   And in middle school, for those being exposed to the Inverse Square Law in 8th grade Physical Science, we have a B-inverted (or BI scale) to allow students a visible way to compute how distance affects the scatter of light. 
...
0.6
>
, the direction will be ~36.9 degrees. Students of yesteryear would know to find the complement of that angle for a 
<0.6,0.8>
<
0.6
,
0.8
>
 unit vector, as well doing the necessary compensation for vectors in other directional quadrants.   
...
Where circled, we divide the 9 by 14 (ignore the decimal in 1.4), the result being read under the left index of the C scale (see under the hairline). However, we do not need this figure (0.642). Instead, we read the bottom, red P scale for the value 0.766. By dividing with the hypotenuse (14), we now have corresponding leg values when the hypotenuse is 1.


From this situation, we can create any number of similar triangles by multiplying all the sides by any value of our choosing. Because we are finding 
x
𝑥
 when the hypotenuse is 14, then 
x
𝑥
 is solved by multiplying the 0.766 value from the P scale by 14. This is shown here, circled, with the left index of the C scale over 0.766 on the D scale, and reading the value of 
x
𝑥
 on the D scale under the 14 (1.4) on the C scale. This value for 
x=10.71
𝑥
=
10.71
 is shown under the hairline.
Solving for 
x
𝑥
 here, technically speaking, is Algebra, but here we have been showing how slide rule usage builds algebraic understanding.  And before you think that we have forgotten about that, let's look at the self-documentation for the P scale.  Easier to see on the Aristo Darmstadt rule, the 
1−
x
2
−
...
Practically speaking, it permits you to see the value of cosine when the rule is set for sine and vice versa.  In the left image above, observe that the rule is set to compute 
sin(40)
𝑠
𝑖
𝑛
...
40
 ...
 by reading the output, 0.643, off of the D scale.   While you could get 
cos(40)
𝑐
𝑜
𝑠
...
40
 ...
 easily enough by moving the cursor, it's not necessary since 
cos(x)
𝑐
𝑜
𝑠
...
𝑥
 ...
 is a Pythagorean relationship through the above identity.  So, you could just read that output from the P-scale!   In this case 
cos(40)=0.766
𝑐
𝑜
𝑠
...
, promoting the co-function trig identities.
​
...
With the K&E Deci-Lon 5 slide rule, the bottom four scales represent one continuous scale for the base of exponentials, beginning at 1.001 on the Ln 0 scale and running to 30,000 on the Ln 3 scale. Flipping the slide rule over reveals Ln-0 to Ln-3 scales for bases between .999 to .00003.


With the Pickett NP4-ES, the scales are implemented differently, with LL1 and LL2 on the top rail and LL3 and LL4 on the bottom, with values above 1 in black numbers and values below 1 in red running "back-to-back" on the same scale.
...
Slide rule makers had their own designs for how their scales were arranged.  Here, because of the massive amount of space on the rule, Aristo put all exponential scales on the same side, with red scales LL00, LL01, LL02, and LL03 on the top rail for exponential bases less than 1 and with black scales LL3, LL2, LL1, and LL0 on the bottom rail for bases greater than 1.   Looking at the right side of the LL3 scale (click the image to zoom in), the self-documentation shows as 
e
x
𝑒
𝑥
 while the C scale is labeled 
x
𝑥
.  The intuition, as taken from the other functions above, is that the number on the C scale is the power of whatever base you set.   The documentation for this uses base 
e
𝑒
 as default, which happens because the slide rule "folds" the scales specially at 
e
𝑒
, or 2.718.  We can see this even with the slide removed (Figure 1, below at left). 
Figure 1 - With the slide removed, the number e (on LL3) is the base of an exponential with 1 (on D) aligned above it, circled. The value is read off of the same LL3 scale. As such, 
e
1
=e
𝑒
...
e
𝑒
 raised to any power can be computed, as under the hairline we have 
e
3
≈20
𝑒
...
e
𝑒
 raised to any power.   Once we add the slide back, we could set the index of C to any base of our choice (on any of the LL scales) and place the hairline on any exponent on that C scale.  The result will be read off the same LL scale where the base was set, unless the right index is used and you would read the output off of the scale above it for bases larger than one and below it for bases less than 1.  
...
Another virtue of using a slide rule to perform these computations is that any base raised to ALL the powers are shown with the single setting of the rule.  Figure 2 at left demonstrates how this can be done using a base of 2.  Again, this is akin to inputting the 
2
x
2
𝑥
 function in a graphing calculator and selecting the table to display a range of outputs, or all of the ordered pairs that make such a function true. 
...
And then, just as division can be performed on the rule due to being the inverse of multiplication, so can logarithms of any base.   For example, for a problem like 
x=lo
g
2
4.5
...
2
4.5
,  the student can set 2 on the LL2 scale as usual, move the hairline to 4.5 on the LL3 scale, and then read the correct answer off of the C scale (exactly the opposite of doing the exponential).  This also reinforces the definition of the log itself, meaning that each time a student solves a log equation, the student is reminded exactly what a "log" is, as the word sentence, "2 to what power is 4.5," is visibly shown on the slide rule. 

This only scratches the surface of understanding for students.  Slide rules require students to develop a need for math properties to solve certain problems.  For example, while a calculator can handle something like 
log(.041)
𝑙
𝑜
𝑔
...
, the slide rule has difficulty with this without a little bit of rethinking.  This is because the LL1 scale (where the base 10 is) and the LL03 scale (where .041 is) cannot be used simultaneously like we did in the previous paragraph.  So students of the era knew that when taking the 
log(x)
𝑙
𝑜
𝑔
...
<
1
, the problem had to be rewritten.  For example, the argument of the log above, .041, could be expressed as a product of 
4.1×
10
−2
4.1
...
−
2
, whereas the "Product Rule of Logs" allows for 
log(4.1)+log(
10
−2
)
...
−
2
, which is something students of the past would have known without using the "power rule" for logs, then the result can be found by computing 
−2+log(4.1)
−
2
+
...
.   Students could use either the L scale or the LL3 scale to compute the log portion (see Figures 3 to 5 below).  And, of course, this was also a good way to get students some practice with scientific notation!
...
Figure 3 - With the index of C set to 4.1 on the D scale, this cute Post/Hemmi 1441 slide rule is setup to compute 
log(4.1)
𝑙
𝑜
𝑔
...
4.1
 ...
 can be found with the Log Log scales, as shown here on this Post 1461 Versalog, which is the more powerful, bigger brother to the Post 1441. We see the index of C setup on the 10 of the LL3 scale (circled). The hairline is set to 4.1, also on LL3. The resulting .613 value, read off the C scale, would add to -2 in the problem, 
log(.041)=log(
10
−2
)+log(4.1)=−2+.613≈−1.37
...
As many things are in education, emphasis on "Cross-Curricular" or "Interdisciplinary" instruction is cyclical.  In fact, it has been quite sometime since this has popped up as my school district's theme for improved instruction, which I fully expect to happen soon.  Quite simply, when subject matters are crossed, it gives additional relevance to both disciplines.  So for a subject like math, where teachers find it extremely difficult to achieve total buy-in from their students, it is helpful to encourage teachers from other areas to incorporate some math within their curriculum design. This is very easy to achieve in the sciences, as so much is already math-based.  Chemistry and Physics, in particular, can benefit with some coordination of activities built to achieve such cross-training of our students.  And there are an amazing number of slide rules that were designed to support this.

But there is a place in almost every academic discipline to utilize a slide rule within its curriculum.  Again, this is not to solve problems, but rather to show students alternative ways to think about problems.  In business, slide rules that were designed for this use are plentiful.  There were slide rules known as "profit calculators," and others for general business use.  In Art/Design, there were "proportional scales" used in the reproduction and rescaling of printed art and documents.   In the trades, there were (and still are) slide rules for hydraulics, pneumatics, and psychrometrics (HVAC). 


Hemmi 257 Chemical Engineering - This slide rule serves a special purpose, as can be seen by the nature of the scale set. Gone are the typical C/D and A/B general math scales, replaced by scales to help chemists. Today's Chemistry teachers would not know that such slide rules exist. Most other disciplines that we teach have analogous slide rules as well.
Every slide rule I have related to fields other than mathematics is an opportunity for kids within other classes.  One of my recommendations is that when you run across a slide rule like this, buy two of them, adding one to your collection and giving the other to one of your teaching partners.  In this way, it gives that teacher an opportunity for, in the least, a "show-and-tell" moment with their students, a connection that will be made back to their math classroom where they've also seen those "sliding thingies."  
...
The classic example is doing the computation 
(3∗4∗5)
(6∗7)
(
3
...
, simplified here to whole numbers for demonstration purposes.   There is danger when doing this computation with a calculator, as students will typically multiply the entire numerator, followed by dividing the denominator.  But as we know, if the parentheses are not honored in the denominator, then the 7 can easily end up being multiplied rather than being divided with the 6.   

​With the slide rule, if the numerator is multiplied first, followed by the denominator, then those intermediate values will need to be written down prior to finding the final quotient.  But there's a trick...students of yesteryear would have known to work the problem as 3 
÷
÷
 6 
×
...
×
×
 5, in that order.  This chaining of operations, which is totally permissible, allows the results of the each step to be used directly into the next operation, all without ever needing to use paper.  

Important for us, and today's students, is that you would never have reason to handle the problem that way, and so students would likely not ever know that doing so was possible, and in many instances both safe and useful.  In this case, the parentheses "serve" the problem, yet I find that students will almost always perform the parentheses quite literally.   As such, they think of the computation as 
60
42
60
42
...
÷
2
, they almost always fail to understand that all they need to do is place the 2 on the bottom - anything on the bottom is divided - leaving 
π
4
𝜋
4
.  Instead, I watch students reciprocate the 2 and then multiply, almost without exception.  
...
Another example of when the slide rule makes you rethink math is what happens when you multiply something like 4.2 
×
×
 6.7.  It requires using the right index on the C scale to get the correct result; however, the inclination is to use the left index first, which would put the solution off the slide rule.   To fix this, you either have to remember to use the right index OR you use an inverse (CI) scale and treat the problem like it's a division problem, because multiplying two numbers is the same as dividing by its inverse, something that would have been common knowledge in the slide rule era. 

Likewise, there are also what is called "folded" scales on a slide rule, as we talked briefly about earlier in the article.   Such scales, usually labeled "CF" and "DF,"  are essentially the same as the C and D scales, yet the beginning of the scales through pi (or 
10
−
−
√
...
The concept of factoring begins early, around the 5th or 6th grade, as students first learn that numbers can be written into products.  In first use, a number can be expressed as its "prime factors."   As example, the number 20 in its prime factored form would be expressed as 
2×2×5
2
×
2
×
5
.   This idea of turning a number into a product of smaller values has important implications for reducing fractions, since if both the numerator and denominator are broken into their prime factors, then the fraction might be reducible.   That could be shown with something like 
12
20
=
2×2×3
...
3
5
.  All the twos would cancel each other here. 

This idea of "forming a product" is key to factoring at higher levels, such as for factoring a math expression.   So, for something like 
2
x
2
+8x
...
8
𝑥
, a product can be formed by pulling the greatest common factor (GCF) - or any factor really - typically learned first around 7th or 8th grade.  In our example, 
2
x
2
+8x=2x(x+4)
...
.  Note that what is left is a product of 
2x
2
𝑥
 and 
...
This takes us to an addition problem like 
422+391
422
+
391
.  As we said, addition problems cannot be performed on a slide rule with algorithmic scales.  However, I would suggest 3 ways to do it using the "spirit" of what we've discussed in this article, with the understanding that this is educational, not practical...

Method 1:  First, related to "forming a product," if the problem can be factored, then a slide rule can be used.   So, for  
422+391
422
+
391
, we can factor out 422.  To do this, 422 becomes the first part of the product when "pulled to the front."  The second part of the product divides 422 into each part of the binomial, or 
(
422
422
+
...
Figure 1 - A DIY Addition Rule made with two linear scales to perform addition and subtraction. Note that each inch is divided into tenths, not the typical standard U.S. or British imperial measurement.
Figure 2 - Our slide rule here is setup to compute something easy, like 
22+45
22
+
45
. Clearly the linear distances add quite easily to yield the sum of 67 on the upper rule. Subtraction is of course the opposite, where the two numbers to be subtracted are aligned over each other and the difference taken off of the left index.
Figure 3 - Performing 
422+391
422
+
391
 from the factoring method above, our slide rule gives an approximate sum of 813, with a little imagination. A lack of precision with this hand-drawn rule not withstanding, the method remains instructive.
Figure 4 - When the sum has "carry-over" digits, spilling into a 4 digit sum, the right index can be used just as with our regular slide rule. Here, the DIY rule is setup to compute 
714+847
714
+
847
...
Applying the newly made rulers to something easy, like 
22+45
22
+
45
, it is clear that summing the distance, like a typical slide rule, yields the correct sum of 67, as shown in Figure 2.  Here I use a yellow "post-it" note as my hairline.  

Precision can come into question with our DIY Addition Rule.  In Figure 3, we show the setup for 
422+391
422
+
391
, as we computed in the previous method.  The sum of 813 can be shown once again with my make-shift cursor. 

Finally, we answer the question you might have wondered, "How does it work for answers that go off the slide rule?"  And just like a regular slide rule, we can simply use the RIGHT index (10) to do such computations.  Working out a sum like 
714+847
714
+
847
 in Figure 4, I have aligned the right index of the bottom scale with the value of 714 on the top scale.  Using a cursor glass from a real slide rule, I set the hairline on 847 on the bottom scale, reading the sum of ~1560 from the top scale.  Here, you would have to know to read the top scale as 1560, and not 560.  4 significant figures of precision is a difficult ask for a DIY rule such as this, since the actual sum is 1561, but it's easy to be amused at what can be accomplished with couple of quickly drawn scales like this.  And for sums no larger than three digits, this method is surprisingly accurate.
...
Even so, if you have taught elementary math, while never having used actual rocks and buckets, then shame, shame...I refer you back to our discussion on manipulatives. 


Faber-Castell 67/54 RB Darmstadt - Showing off its slide rule side.


Faber-Castell 67/54 RB Darmstadt - Showing off its Addiator side.
Where to Start?
...
Moreover, slide rule technique is a reminder to educators that math is not about computations, but rather the study of algorithms, patterns, and techniques that might be capable of helping us solve problems.  This fact needs to be communicated continually to students, parents, and decision-makers within education, as most people operate under the mistaken impression that math is somehow about solutions rather than being process-oriented.   And if you, having read this, never touch a real slide rule, yet become a more effective teacher because of your understanding and advocacy of this, then I will have succeeded. 

That being said, by now you undoubtedly wonder how I use slide rules in my own high school math classroom?  Let's sort through these items in bulleted fashion...

Because I collect slide rules, my classroom is used to display my collection.  One wall is almost completely filled with around 350 of the slide rules in my collection.  These slide rules are out of their cases, hanging on custom 3D printed hangers that are hot-glued to my wall.   The slide rules range from basic to complex, less expensive to quite valuable, with a diversity in appearance that is likely more at home in a museum.   

Speaking of which, I also exhibit some desk models (on my desk), a Thatcher calculator, and two Fuller calculators on my book shelves.   At the front and back of the classroom hang giant "demonstrator" slide rules that were once made by slide rule makers to facilitate training in an education or industrial classroom setting.  

This provides a setting that sparks curiosity within students, prompting them to ask math-related questions, as well as to get students to see that they have a teacher that not only loves what he teaches, but is willing to put his own money behind it.

I provide opportunities for my own students to learn and use a slide rule as extra credit.  While you might believe they do so reluctantly, I think you might be surprised how many students just think they are "cool."  The hook is actually quite simple...I tell them I can do 
x
𝑥
 with a slide rule and that sparks a curiosity.   I provide a research project where they can earn points and I give them a slide rule for their efforts.   This academic year to date, in 2022-23, I have given away 33 slide rules.   That is 33 students who gained a deeper understanding of mathematics and an appreciation of mathematical history, while utilizing time outside of the classroom.  

Many of the slide rules intended for student use have terrific conversion tables and formulas on the slide rule itself.  While these were designed to provide a basis for computations when using a slide rule, they do not have to be.  In many cases, the tables themselves are worth their own weight in gold.  Where applicable, I often place a slide rule into a student's hand rather than a paper formula chart.   I could have very well expanded this into a whole section within the article, but ten sections was sufficient enough!

Lastly, while there is nothing required of my students involving slide rules (they are not in our state's educational standards), I often utilize the slide rule within teacher-led instruction, as demonstration of some concepts as it relates to various units of study.  For example, it's easy to demonstrate what logarithmic scales are during our unit on Exponential and Logarithmic Functions.  Or, to illustrate a table of values for quadratic and cubic functions (or their roots) by showing how they perfectly line up on the A and K scales respectively.   

And, importantly, because many of my slide rules are "specialty rules" in nature, they can be used to illustrate parallel applications, such as using a "vector" slide rule to compute reactance within electronics (within our Vectors unit) or even showing rectangular to polar conversion within our unit on Parametric and Polar Equations.  Even if the students never touch a slide rule, it shows them that what is being studied has real-life importance and application, something that did NOT stop once slide rules became extinct. 
...
Many of the best slide rules for student use today are the same as those that were marketed for student use back in the 1960s and 1970s.  These rules, while completely functional, were directed toward wide-distribution among budget-minded consumers.  For this reason, they can be found today at reasonable prices in bulk, mostly on eBay.  

Many of the slide rules pictured within the article are not such slide rules, but rather some "collectors' favorites," usually of more expensive construction, and always carrying a collector's price tag.  But even those, like the bamboo Post Versalog and mahogany K&E 4081-3 Log Log Duplex can be found for as little as $20.  So while I do not seek to have large numbers of such rules for student use, they are slide rules that I recommend for students when they inquire about purchasing their own; which are rules that have to be felt "in the hand" to be really appreciated.   You will never feel the same way about using a calculator!

Suitable slide rules found in larger numbers that my students are allowed to handle (many of which I give away), are those that often have education-friendly words written on the rule itself, often branded as a "trainer," "scholar," or "student" rule.   They will typically be plastic in construction, often single-sided rules with useful conversion formulas on the back side.  The chief makers of these rules were mostly American companies.  Two notable brands include Sterling and Acumath, neither of which made anything other than a student-type of slide rule. So any rule you find from these makers should be budget-oriented. 

Larger American makers, known for high quality slide rules, also had lines of budget rules.  These include any plastic Pickett rule, designed with a Model 1XX label (such as the Model 120, 140, and 160), and K&E with their K12-Prep rule.  The only European maker I've found with rules suitable to buy cheaply in bulk is Aristo, specifically their rules with 90X model numbers, though these will typically ship internationally from Germany.  Shipping is cheap to the States (unlike England), but you just need to be more patient with delivery.  

Many other slide rules qualify for worry-free student use, though are typically found in singles.  It makes more sense to save on shipping by buying in bulk; though it should be said that many online sellers will sell larger mixed lots of cheaper slide rules that can be more than suitable.  Likewise, estate sales can produce good finds, though rules from collectors will tend toward the nicer slide rules that may, or may not, be something you want students to handle.  

Below I show the front and back sides of many of the student rules I recommend.  Click on the images to zoom.  They can be found in the $5 range if you look hard enough.  Just be careful about shipping a single slide rule, as that will more than double the price of these slide rules.
Aristo 903 Scholar - Front
Aristo 903 Scholar - Back
Pickett 121 Microline - Front
Pickett 121 Microline - Back
Sterling Precision - Front
Sterling Precision - Back
Acumath 600T - Front
Acumath 600T - Back
Creating an environment for kids to learn math could be as simple as posters on the wall or student work on display. I prefer to surround them with a display of mathematics' past. My collection of slide rules adorns the back wall. Most collectors, in an attempt to preserve them, keep slide rules locked out of sight in their cases. I prefer to use and admire them.
